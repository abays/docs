...
| **swift-operator** | Object storage data (`/srv/node/pv`) | Yes | Contains actual Swift objects (user data). PVC labels set at VolumeClaimTemplate creation + reconcile for existing PVCs. Restore order 00 (PVCs). **Note:** Swift stores object metadata in extended attributes (xattrs) which kopia does not preserve. A Velero pre-backup hook dumps xattrs before the CSI snapshot, and an init container on the StatefulSet restores them before Swift services start. See user-guide Step 4 and [swift-operator docs](https://github.com/openstack-k8s-operators/swift-operator/blob/main/docs/backup-restore-xattrs.md). |
